...
//(Future Dev? Low Voltage wire should (cat 5/e) should not be run next to 120/240.  Code violation and causes interference)
//-- end features
...
#include <SPI.h>
#include <Ethernet.h>
byte mac[] = {  0xDE, 0xAD, 0xBE, 0x01, 0x56, 0xED };
      ...
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          // output the value of each analog input 
            client.println("Some type of status output<br />");
            client.println(g_EvseController.GetState());
